...
def dns_to_ip(request):
    ...
    try:
        ...
        resp.addEntity(IPAddress, ip_address)
    except socket.error as e:
        resp.addUIMessage("Error: " + str(e), UIM_TYPES["partial"])

    # Write the slider value as a UI message - just for fun
    resp.addUIMessage("Slider value is at: " + str(request.Slider))
...
def phrase_to_as(request):
    ...
    if not request.Value.isdigit():  # Ensure the input value is a digit
        resp.addUIMessage("Input value '%s' is not a whole number." % request.Value, UIM_TYPES["partial"])
        return resp.returnOutput()
    else:
    ...
    if not is_div_str.isdigit():  # Ensure is_div setting is a digit
        resp.addUIMessage("ISDIV setting value '%s' is not a whole number." % is_div_str, UIM_TYPES["partial"])
        return resp.returnOutput()
    else:
    ...
    return resp.returnOutput()  # return the XML to the TDS server