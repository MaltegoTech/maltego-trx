...
from .maltego import MaltegoRequest
from .server import print_transforms, run_transform, get_exception_message, registry
...
def handle_run(name, args, app, port=8080, ssl_context=None, debug=False, ssl_keyfile=None, ssl_certfile=None):
    if name == "__main__":
        if len(args) >= 2:
            ...
            if command == "runserver":
                ...
                print_transforms()
                uvicorn.run(app, host="0.0.0.0", port=port, debug=debug, reload=debug, ssl_keyfile=ssl_keyfile, ssl_certfile=ssl_certfile)
                ...
                if transform_name in registry.transform_classes:
                    client_msg = MaltegoRequest.from_local_args(args[3:])
